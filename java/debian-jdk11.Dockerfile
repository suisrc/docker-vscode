...
RUN echo "**** install AdoptOpenJDK ****" &&\
    ...
    mkdir -p /usr/lib/jvm/java-11-adopt &&\
    curl -L ${JAVA_URL} -o /tmp/adopt-open-jdk.tar.gz &&\
    tar -xzf /tmp/adopt-open-jdk.tar.gz -C /usr/lib/jvm/java-11-adopt --strip-components=1 &&\
    ln -s /usr/lib/jvm/java-11-adopt/bin/java /usr/bin/java &&\
    rm -rf /tmp/* &&\
    # smoke tests
    java -version

ENV JDK_HOME=/usr/lib/jvm/java-11-adopt
ENV JAVA_HOME=/usr/lib/jvm/java-11-adopt
